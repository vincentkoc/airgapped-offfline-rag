...
@patch('app.model_handler.Llama')
def test_generate_llama(mock_llama, model_handler):
    ...
    result = model_handler.generate("Test prompt", model_choice="llama")
...
@patch('app.model_handler.Llama')
def test_generate_mistral(mock_llama, model_handler):
    mock_mistral_instance = MagicMock()
    mock_mistral_instance.return_value = {
        'choices': [{'text': 'Generated text'}]
    }
    mock_llama.return_value = mock_mistral_instance

    result = model_handler.generate("Test prompt", model_choice="mistral")
